...
def fake_batch(batch_size: int) -> list[Event]:
    def _fake_event(event_type: str) -> Optional[EventInfo]:
        if event_type == 'rating':
            event = randint(1, 100)
            return EventRating(movie_id=str(uuid4()), user_id=str(uuid4()), event=event, event_time=datetime.now())
    ...
    for _ in range(batch_size):
        ...
        _event: Event = Event(event_type=_event_type, event_payload=_fake_event(_event_type))
        if _event:
