...
class KafkaClientETL:
    def __init__(
        ...
        self.client = AIOKafkaConsumer(
            *topics, bootstrap_servers=host, group_id=group_id, auto_offset_reset='earliest', enable_auto_commit=False
        )
    ...
    def _transform_to_message(record):
        ...
        timestamp = value
        return ViewsMessage(user_id=user_id, film_id=film_id, timestamp=timestamp)
...
async def main():
    kafka_client = KafkaClientETL(host='localhost:29092', topics=['views'], group_id='gid', batch_size=4)
